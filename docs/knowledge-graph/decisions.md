...
**Decision:** `HistoricalEffectiveEncounter` is the canonical no-ledger-replay projection of original evidence, current overrides, effective doctor/procedure/sedation/Add-on/allocation, correction support, disposition, reason/source, and revision. Accepted Ready or terminal Ready is original assignment authority when truthful; otherwise the typed source contributes only facts it durably stores. Issue #241 is the reporting and Data Quality consumption point, and #242 integrates that projection into the existing Reports audit area.

**Decision:** Reporting persistence projects canonical historical truth set-wise. Fixed completed and aborted pages join valid Ready evidence and current administrative state, while indexed ledger `EXISTS` checks provide durable correction and reviewed provenance without per-encounter effective-projection or ledger reads. The internal `HistoricalReportingProjection` is applied only to detached report/audit rows before scope, derived reporting-exception annotation, grouping, occupied-wait attribution, and calculations. Database all-time counts and paged audit predicates use the same effective doctor, procedure, explicit sedation, and disposition rules.

**Decision:** Canonical Ready-backed and corrected sedation uses explicit effective `SedationState`, independent of procedure suffix transport. Only legacy completed history without truthful Ready or explicit sedation evidence may retain `+SED` or legacy `SED` classification for ordinary normal reporting. Scoped anomaly and Data Quality membership requires explicit effective sedation; unknown satisfies neither Sedation nor NonSedation. Historical assigned Schedule Fit uses current effective explicit historical allocation, while current-default Calibration continues to use today's active base-procedure roster default.

**Decision:** Reports owns one integrated anomaly review and history surface. `AnomalyReview` status filters select Needs Review, Confirmed Exception, Cleared for Reporting, or all three current anomaly dispositions while preserving active report scope. A separate explicit control broadens to All Time, Practice, unrestricted Doctor, All Sedation, and no procedure restriction. Selecting one typed encounter loads its canonical detail and bounded ledger on demand; shared correction options expose complete active and inactive governed rosters without free-text or synthetic `+SED` identities.

**Decision:** The #242 browser distinguishes immutable lifecycle/original Ready evidence, current effective metadata, and current administrative interpretation. Needs Review alone exposes corrections, notes, Clear, and Confirm; resolved encounters remain inspectable with Reopen. Every mutation uses the current revision. Stale writes and unknown transport outcomes are reconciled by reads and never auto-retried. Retained older exception routes are compatibility-only and are not called or advertised by the checked-in browser.
