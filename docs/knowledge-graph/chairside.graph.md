...
- The operator reset wrapper code-owns only the canonical Training app, data, backup, app-pool, and child-environment values; it backs up the stopped-pool SQLite file set, preserves the original Started or Stopped app-pool state, refuses every other state before backup, and delegates mutation to existing marker-validating maintenance commands.
