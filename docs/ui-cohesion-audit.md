...
- **Room workflow clarity.** The primary/secondary/danger/available button hierarchy on the room panel, and the visual separation of the `.corrections-panel` (Update Assignment / Cancel Seating) from the primary workflow buttons, encode which actions are safe pre-arrival corrections vs. the main lifecycle sequence. Do not visually flatten this hierarchy into uniform chip buttons.
