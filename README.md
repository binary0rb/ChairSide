...
- `scenario-rich` - an extended multi-month completed-cycle history plus a small set of explicit edge-case cycles (one clean cycle in each of the Today, Last-7, Last-30, and older-than-Last-30 report windows; one cycle per derived reporting-exception reason; one manual audit-review candidate; and a deterministic same-doctor three-room overlap), for reporting/trend/exception-review/concurrency testing.
